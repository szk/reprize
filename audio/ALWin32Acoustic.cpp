...
#define ALCHECK(N, n) if (!(ALfuncTbl->n=(N)GetProcAddress(g_hOpenALDLL,#n))) \
    { \
        RE_DBG("Failed to retrieve '" #n "' function address"); \
        return AL_FALSE; \
    }
...
//     if (!init())
//     {
//         RE_DBG("Failed to initialize OpenAL" << std::endl;
//         return;
//     }
