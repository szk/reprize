...
GLX11FontLoader::GLX11FontLoader(X11DepInfo* depinfo_)
...
GLX11FontLoader::~GLX11FontLoader(void)
{
}

void GLX11FontLoader::init(void)
{
}

const bool GLX11FontLoader::load(const Str& filename_, const Str& path_)
{
...
//     FT_RENDER_MODE_MONO, FT_RENDER_MODE_NORMAL
    ...
    if(dest_width && dest_height && tex_loader)
    {
        uInt32* buf = new uInt32[dest_width * dest_height];
        if (bmptobuf(bitmap.buffer, buf, 8 * 8))
        { loaded_node = tex_loader->build("ttf_test", buf, 3); }
        delete[] buf;
    }
...
const bool GLX11FontLoader::bmptobuf(uChar* src_, uInt32* dst_, uInt32 size_)
{
    for (Size32 i = 0; i < size_; ++i) { dst_[i] = 0xFFFF00FF; }
    return true;
}

Glyph* GLX11FontLoader::make_tex(const Str& filename_, Int32 x_, Int32 y_,
...
Glyph* GLX11FontLoader::make_geom(const Str& filename_, uInt32 code_)
{
    FT_Face ft_face;
...
void GLX11FontLoader::tess_begin(GLenum type_, Geometry* geom_)
{
    cerr << "b";
//     mesh->Begin( type);
}

void GLX11FontLoader::tess_end(Geometry* geom_)
{
    cerr << "E";
//      mesh->End();
}

void GLX11FontLoader::tess_vertex(void* data_, Geometry* geom_)
{
    cerr << "v";
...
void GLX11FontLoader::tess_combine(GLdouble coords_[3], void* d_[4],
...
void GLX11FontLoader::tess_error(GLenum err_, Geometry* geom_)
{
    cerr << "e";
//     mesh->Error( errCode);
}

int GLX11FontLoader::move_to(FT_Vector* to_, Geometry* geom_)
{
    geom_->add_mesh(LINE_LOOP);
    geom_->add_mesh_idx(geom_->get_vertex_n());
...
int GLX11FontLoader::line_to(FT_Vector* to_, Geometry* geom_)
{
    geom_->add_mesh_idx(geom_->get_vertex_n());
    geom_->add_vertex(to_->x, to_->y, 0);
    return 0;
...
int GLX11FontLoader::conic_to(FT_Vector* ctrl_, FT_Vector* to_, Geometry* geom_)
{
...
int GLX11FontLoader::cubic_to(FT_Vector* ctrl1_, FT_Vector* ctrl2_,
